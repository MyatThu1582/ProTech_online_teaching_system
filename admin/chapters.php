        ...
        <div class="card">
          <div class="card-body">
            <table class="table">
              ...
              <tbody>
                <?php
                $chapterstmt = $pdo->prepare("SELECT * FROM chapters ORDER BY course_id DESC");
                $chapterstmt->execute();
                $chapterdatas = $chapterstmt->fetchAll();
                $i = 1;
                foreach ($chapterdatas as $chapterdata) {
                  $course_id = $chapterdata['course_id'];
                  $course_idstmt = $pdo->prepare("SELECT * FROM course WHERE id='$course_id'");
                  $course_idstmt->execute();
                  $course_iddatas = $course_idstmt->fetch(PDO::FETCH_ASSOC);
                  ?>
                  <tr>
                    <td><?php echo $i;?></td>
